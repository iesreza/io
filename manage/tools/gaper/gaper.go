...
func run(cfg *Config, chOSSiginal chan os.Signal, builder Builder, runner Runner, watcher Watcher) error {
	...
	for {
		...
		case event := <-watcher.Events():
			fmt.Println("Detected new changed file:", event)
			if changeRestart {
				fmt.Println("Skip restart due to existing on going restart")
				continue
			}
		...
		case err := <-runner.Errors():
			fmt.Println("Detected program exit:", err)
...
func restart(builder Builder, runner Runner) error {
	...
	if err := builder.Build(); err != nil {
		fmt.Println("Error building binary during a restart:", err)
		return nil
	}

	if _, err := runner.Run(); err != nil {
		fmt.Println("Error starting process during a restart:", err)
		return nil
	}
